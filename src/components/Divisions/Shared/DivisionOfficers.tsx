...
import { type Officer, divisionOfficerMap } from '@/../config/officers.config';

type Props = {
  division: Division;
};

function getDivisionDirectors(division: Division): Officer[] {
  const divisionOfficers = divisionOfficerMap[division] ?? [];

  return divisionOfficers.filter((officer) =>
    /director/i.test(officer.position)
  );
}

export default function DivisionOfficers({ division }: Props) {
  const directors = getDivisionDirectors(division);
  return (
