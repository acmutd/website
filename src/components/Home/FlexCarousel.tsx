...
export function FlexCarousel({ items }: FlexCarouselProps) {
  ...
  return (
    <div className="background-container relative mx-12 md:mx-3 lg:mx-auto flex w-full lg:max-w-[50vw] flex-col gap-y-6 rounded-3xl border border-primary/50 bg-amber-600/10 p-3 pb-16 backdrop-blur-xl">
      <div className="relative w-full">
        <Carousel className="w-full" setApi={setApi} opts={{ loop: true }}>
                  ...
                  <DialogTrigger asChild>
                    <div className="relative aspect-[16/9] cursor-pointer">
                      <Image
                        ...
                        alt={item.title}
                        fill
                        className="object-cover rounded-[40px] transition-transform"
                      />
                    </div>
                  </DialogTrigger>
          ...
          <div className="absolute bottom-0 left-1/2 -translate-x-1/2 flex justify-center gap-2 py-2 px-4 bg-black/20 backdrop-blur-sm rounded-full">
            {items.map((_, index) => (
              <button
                key={index}
      ...
      <div className="absolute bottom-0 right-0 bg-white/10 px-4 py-2 rounded-tl-xl border-t-2 border-l-2 border-white/50 z-10">
        <p className="text-lg text-white font-sans">{items[currentIndex]?.title}</p>
      </div>
    </div>
  )
}
